...
print_usage() {
    script=$(basename "$0")
    >&2 echo "Usage: "
    >&2 echo "$script -s seed -c config yml"
}
...
LOG_DIR="$SAVE_DIR_PREFIX/seed_$SEED/logs/$DENOISING_ID"
PRETRAIN_SAVE_DIR="$SAVE_DIR_PREFIX/seed_$SEED/pt/fairseq/$MODEL_CONFIG-$TASK-$DENOISING_ID"
CONVERT_SAVE_DIR="$SAVE_DIR_PREFIX/seed_$SEED/pt/hf_conv/$MODEL_CONFIG-$TASK-$DENOISING_ID"
FINETUNE_SAVE_DIR="$SAVE_DIR_PREFIX/seed_$SEED/ft/$MODEL_CONFIG-$TASK-$DENOISING_ID"
...
id_generate=$(
    ...
    -m "$FINETUNE_SAVE_DIR" -b 120 -o "$FINETUNE_SAVE_DIR"
)
...
#     $SLURM_ARGS_GENERIC \
#     $SLURM_LOG_ARGS \
#     $BASE/pretraining/convert_model.sh \
#     "$PRETRAIN_SAVE_DIR" \
