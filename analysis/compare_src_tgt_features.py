...
for col in df.columns:
    if col.startswith('src') and col.endswith(feature_type):
        # print(f'{col} vs. TGT')
        data[col] = compute_sim(df[col], df[f'tgt_{feature_type}'])
...
for i in range(1, max(spkr_preds_summed) + 1):
    ...
    data[f'Consistency >={i}'] = compute_sim(src_ind, df[f'tgt_{feature_type}'])
...
for i in range(1, max(opp_spkr_preds_summed) + 1):
    ...
    data[f'Mirroring >={i}'] = compute_sim(src_ind, df[f'tgt_{feature_type}'])
...
for i in range(1, max(src_preds_summed) + 1):
    ...
    # print(f'SRC (>={i}) vs. TGT')
    data[f'Existence >={i}'] = compute_sim(src_ind, df[f'tgt_{feature_type}'])

# for i in range(1, max(src_preds_summed) + 1):
print(f'\nMajority')
# breakpoint()
src_ind = [1 if x >= 3 else 0 for x in src_preds_summed]
data['Majority'] = compute_sim(src_ind, df[f'tgt_{feature_type}'])

results_df = pd.DataFrame.from_records(data).T
print()
